\ color.f       - console colour handling
\ ------------------------------------------------------------------------
...
\ props to the developers of terminfo for just sucking it up and adding
\ a horrendous ammount of duct tape code to the library simply to account
\ for all the idiotic decisions made by all the people who create
\ these terminals.  most of them are so badly written NOBODY wants to
\ maintain any part of them!

  <headers

: (>pref)       ( c1 --- )
  sgr0                      \ clear all attribs
  dup :standout:  and if smso then
  dup :underline: and if smul then
  dup :reverse:   and if rev  then
      :bold:      and if bold then ;
