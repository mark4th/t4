...
\ ------------------------------------------------------------------------
  ...
  3061 const #BLOCKS        \ max number of allocations per heap

\ ------------------------------------------------------------------------

    85 const MAX-HEAPS      \ maximum number of 20 meg heaps
...
\ allocated block or a free block but which now describes neither.
\ unused blocks are cached for later reuse

struct: mem-blk             \ this is a descriptor for a memory fragment
  /node: b.link             \ mem-blks are stored in linked lists
...
struct: heap
  /node: h.link             \ linked list entries
...
\ ------------------------------------------------------------------------
\ meta data at start and end of block

struct: meta
  1 dd m.blk                \ address of mbm-blk descriptor
  1 dd m.magic              \ magic number
  1 dd m.reserved           \ not used
  1 dd m.sum                \ checksum
;struct
...
\ ------------------------------------------------------------------------
\ markers placed in region guard blocks to indicate allocated state
