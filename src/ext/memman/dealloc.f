...
: (merge)     ( mem-blk1 mem-blk2 --- mem-blk1 )
  ...
  over b.size +! ;          \ grow lower block by size of upper block
...
: merge>      ( mem-blk --- mem-blk )
  ...
  if                        \ if so get its mem-blk and unlink from map
    (@meta) <list (merge)   \ merge upper retion into lower region
...
: (free)        ( mem-blk --- f1 )
  ...
  if                        \ if they are the same
    drop r> destroy-heap    \ return entire heap pool to BIOS
  else                      \ otherwise
    r> h.mapf@              \ link deallocated block to heaps
    swap add-free           \ free blocks mem-map
  then
  true ;                    \ return success
