...
: wemit         ( c1 win --- )
  over $0a =                \ allows emit of a cr to the window
  if
    nip win-cr exit
  then
  over $0d =                \ neuter this char though
  if
    2drop exit
  then
  dup>r (wemit)             \ write char
  r> cursor-rt ;            \ advance cursor
